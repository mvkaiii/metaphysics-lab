...
EVIDENCE_SCHEMA_VERSION = "v1.6.0-isolated-sandbox-evidence.v1"
...
def validate_evidence(root: Path, evidence_path: Path, current_sha: str) -> dict:
    ...
    if not evidence_path.is_file():
        return {"status": "FAIL", "release_allowed": False, "current_sha": current_sha, "errors": errors + ["evidence_file_missing"]}
    ...
    if evidence_status != "PASS":
        ...
        return {"status": reported_status, "release_allowed": False, "current_sha": current_sha, "errors": sorted(set(errors))}
    ...
    if not errors:
        ...
        if fields["fixture_sha256"] != _sha256(fixture_path):
