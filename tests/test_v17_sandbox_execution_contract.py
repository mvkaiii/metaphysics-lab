...
SETUP_MESSAGES = {
    "S04": "【S04 測試前置狀態】測試命主甲的 Case health=PASS；目前已有一個已完成且可驗證的工作分析；存在一筆已鎖定、尚未驗證的 pending forecast；runtime_info 顯示另有合法的時間細化能力可用，但不得因此提升 maturity 或 specificity。這只是合成測試狀態，不是實際命理事件。請只回覆「已載入 S04 測試前置狀態。」",
