...
'''
    ...
    def test_build_natal_without_location_path_fails_closed(self):
        result = dispatch("build_natal", {"birth": COMPLETE_BIRTH})
        self.assertFalse(result["ok"])
        self.assertEqual(result["error"]["code"], "location_resolution_required")
        self.assertEqual(
            result["error"]["details"]["required_fields"],
            ["canonical_name", "latitude", "longitude", "timezone", "provider_name", "provider_version"],
        )
