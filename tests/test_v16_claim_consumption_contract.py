...
class ClaimConsumptionContractTests(unittest.TestCase):
    def test_direct_same_domain_convergence_can_render_without_extra_caveat(self):
        p = packet(
            "career",
        ...
        self.assertEqual(result["profile_version"], CLAIM_CONSUMPTION_PROFILE_VERSION)
        self.assertTrue(result["claim_consumption_digest"])
        self.assertEqual(decision["decision"], "render")
        self.assertEqual(decision["authorized_specificity"], "concrete_event")
        self.assertEqual(decision["reason_codes"], [])

    def test_layered_complement_is_rendered_with_caveat_without_creating_target_claim(self):
        p = packet(
            "career",
            bazi=[evidence("bazi", "b1")],
        ...
        self.assertEqual(decision["claim_id"], "claim:yearly:career")
        self.assertEqual(decision["decision"], "render_with_caveat")
        self.assertEqual(decision["reason_codes"], ["layered_complement"])

    def test_disjoint_domains_remain_parallel_and_independently_consumable(self):
        packets = [
            packet(
                "career",
        ...
        self.assertEqual(
            {row["primary_domain"] for row in result["decisions"]},
            {"career", "finance"},
        )
        self.assertEqual(
            {row["coordination_relation"] for row in result["decisions"]},
            {"parallel_signals"},
        )
        self.assertEqual(
        ...
        self.assertEqual(
            {tuple(row["reason_codes"]) for row in result["decisions"]},
            {("parallel_signals",)},
        )

    def test_single_system_support_is_caveated_without_inventing_cross_system_support(self):
        p = packet("career", bazi=[evidence("bazi", "b1")])
        r = coordination_row("career", "single_system_support", systems=("bazi",))
        decision = build([p], [r])["decisions"][0]
        self.assertEqual(decision["coordination_relation"], "single_system_support")
        self.assertEqual(decision["decision"], "render_with_caveat")
        self.assertEqual(decision["reason_codes"], ["single_system_support"])

    def test_tampered_claim_bundle_digest_fails_closed(self):
        p = packet("career", bazi=[evidence("bazi", "b1")])
    ...
    def test_valid_source_bundles_with_mismatched_ranking_provenance_fail_closed(self):
        p = packet("career", bazi=[evidence("bazi", "b1")])
        r = coordination_row("career", "single_system_support")
        with self.assertRaises(ValueError):
            build_claim_consumption_bundle(
    ...
    def test_coordination_cap_can_only_downgrade_authorized_specificity(self):
        p = packet(
            "career",
            bazi=[evidence("bazi", "b1")],
            ziwei=[evidence("ziwei", "z1")],
        ...
        decision = build([p], [r])["decisions"][0]
        self.assertEqual(decision["original_effective_specificity"], "concrete_event")
        self.assertEqual(decision["authorized_specificity"], "event_family")
        self.assertEqual(decision["decision"], "render_with_caveat")
        self.assertEqual(decision["reason_codes"], ["specificity_downgraded"])

    def test_coordination_cap_more_permissive_than_packet_authority_fails_closed(self):
    ...
    def test_same_source_bundles_are_deterministic_and_decisions_use_canonical_domain_order(self):
        packets = [
        ...
        self.assertEqual(
            first["claim_consumption_digest"],
            second["claim_consumption_digest"],
        )
        self.assertEqual(
            [row["primary_domain"] for row in first["decisions"]],
            ["career", "finance"],
        )

    def test_legacy_conflict_never_overrides_python_parallel_authority(self):
        p = packet(
            "career",
            bazi=[evidence("bazi", "b-car")],
            legacy="conflict_or_divergence",
        ...
        decision = build([p], [r])["decisions"][0]
        self.assertEqual(decision["coordination_relation"], "parallel_signals")
        self.assertEqual(decision["decision"], "render_with_caveat")
        self.assertIn("parallel_signals", decision["reason_codes"])

    def test_builder_does_not_mutate_source_bundles(self):
        p = packet("career", bazi=[evidence("bazi", "b1")])
        r = coordination_row("career", "single_system_support", systems=("bazi",))
        claims = claim_bundle([p])
        ...
        self.assertEqual(claims, frozen_claims)
        self.assertEqual(coordination, frozen_coordination)

    def test_zero_same_scope_target_support_abstains_claim(self):
        p = packet(
            "career",
        ...
        decision = build([p], [r])["decisions"][0]
        self.assertEqual(decision["decision"], "abstain_claim")
        self.assertEqual(decision["reason_codes"], ["no_same_scope_target_support"])
